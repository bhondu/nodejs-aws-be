...
describe('getProductListAvailable', () => {
  test('should return 200 and return a list of 9 products with non-zero count', async () => {
    const event = createMockEvent({ path: 'product' });
    const response = <APIGatewayProxyResult>await getProductListAvailable(event, <Context>{}, () => {});
