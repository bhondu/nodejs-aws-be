...
test('getProductList should return 200 and return a list of 10 products', async () => {
  const event = createMockEvent({ path: 'product' });
  const response = <APIGatewayProxyResult>await getProductList(event, {} as Context, () => {});
